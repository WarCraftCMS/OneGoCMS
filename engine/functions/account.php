...
////////////// A-WoW Freekassa
    public function donate($donationId)
    {
        global $cfgMerch, $cfgSec1, $cfgCur;

        $donation = $this->getSpecifyDonate($donationId);
        if (!$donation) {
            return "Invalid donation ID.";
        }
        ...
        $merchant_id = $cfgMerch;
        $amount = number_format($setTotal, 2, '.', '') * 1;
        $currency = $cfgCur;
        $payment_id = time();
        $us_account = $this->get_id();
        $sign = md5(implode(':', [
            $merchant_id,
            $amount,
            $cfgSec1,
        ...
        header("Location: https://pay.freekassa.com/?m={$merchant_id}&oa={$amount}&currency={$currency}&o={$payment_id}&s={$sign}&us_account={$us_account}&us_id={$donationId}");
        exit;
    }

    public function completeTransaction($paymentId, $userId, $donationId)
    {
        $donation = $this->getSpecifyDonate($donationId);

        $obtainedPoints = $donation['points'];
        $this->updateDonorPoints($userId, $obtainedPoints);

        return "Donation successful!";
    }

    private function updateDonorPoints($userId, $points)
    {
        $stmt = $this->website->prepare("UPDATE users SET donor_points = donor_points + ? WHERE account_id = ?");
        if (!$stmt) {
            die("Database prepare error: " . $this->website->error);
        }
    ...
    private function getSpecifyDonate($id)
    {
        $stmt = $this->website->prepare("SELECT * FROM donate WHERE id = ?");
